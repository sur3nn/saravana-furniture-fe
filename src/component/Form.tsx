...
export default function FurnitureForm(props: FurnitureFormProps) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const data: any = await safeFetch("https://test.tap2bill.in/api/meta/categories");
    ...
    //   const data: any = await safeFetch("https://test.tap2bill.in/api/meta/material");
...
const fetchProductsByCategory = async (index: number, categoryId: string) => {
  ...
  try {
    const data: any = await safeFetch(
      `https://test.tap2bill.in/api/products?categoryId=${categoryId}`,
      {
        method: "GET",
    ...
    setProductOptions(Array.isArray(data?.data[0]?.productDetails) ? data.data[0]?.productDetails : []);
  } finally {
  ...
  const submitForm = async () => {
    ...
    setSubmitting(true);
    try {
      const productMap = products.map((p) => Number(p.productId));
      const payload = {
        name: form.name,
        email: form.email,
        mobile: form.phone,
        comment: form.description,
        productMapId: productMap,
      };

      const res: any = await safeFetch("https://test.tap2bill.in/api/enquiry", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
Modal.success({
  title: "Thank You!",
  content: "We’ll call you soon. Your order has been received.",
  centered: true,
  okText: "Okay",
      ...
      <motion.div className="relative w-full max-w-lg bg-[#F7EFE5] p-6 rounded-2xl shadow-xl border border-[#E8D8C4]">
        ...
        <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2 px-2 scrollbar-thin scrollbar-thumb-[#C6A27B]/60 scrollbar-track-[#F3E9DD]/40">
          ...
          <div className="space-y-4">
                    ...
                    {loadingProducts.includes(idx) ? (
                      ...
                      <select value={p.productId} onChange={(e) => handleProductChange(idx, "productId", e.target.value)} className={`w-full p-2 rounded-lg border ${errors[`product_${idx}`] ? "border-red-500" : "border-[#D3C2AF]"} text-[#5A4632]`}>
                        ...
                        {Array.isArray(productOptions) && productOptions.map((pr) => (
                          <option key={pr.productMapId} value={pr.productMapId}>{pr.productName}</option>
                        ))}
                      </select>
                    )}
           ...
           <div className="py-2">
             <button type="button" onClick={addProduct} className="flex items-center gap-2 text-sm font-semibold font-mono px-3 py-2 bg-[#b1681b] text-white! rounded-lg hover:bg-[#995c16]">
...
<div className="py-2">
  ...
  <textarea
    ...
    className="w-full p-3 h-28 rounded-lg bg-[#F3E9DD] border border-[#D3C2AF] text-[#5A4632] resize-none"
